...
class DefinicoesCest
{
    ...
    public function EditProfile(FunctionalTester $I)
    {
        ...
        $I->fillField('Perfil[apelido]', 'SportGym');
        $I->fillField('Perfil[dtaNascimento]', '1999-12-17');
        $I->selectOption('Genero', 'F'); //css path (untested), if you have a label just use this, see link
        $I->fillField('Perfil[username]', 'admintest');
        $I->fillField('Perfil[telefone]', '922222222');
        $I->fillField('Perfil[nif]', '253352459');
        $I->fillField('Perfil[rua]', 'Rua do Administrador');
        $I->fillField('Perfil[localidade]', 'Leiria');
        $I->fillField('Perfil[cp]', '2490-120');
        $I->click('Gravar', 'button');
